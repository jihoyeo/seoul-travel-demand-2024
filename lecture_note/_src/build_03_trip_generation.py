"""03_trip_generation.ipynb — admdong P_obs/A_obs + 회귀 (통행 수요 메인 시작)."""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from nbgen import MD, CODE, write_both

cells = [
MD("""# Lecture 3 — 통행 발생 (Trip Generation)
...
MD("""## 3. admdong 경계 로드 + 두 코드 체계 변환표 만들기"""),

CODE("""boundary = gpd.read_file(os.path.join(RAW, 'admdong_boundary', 'admdong_2023.geojson'))
...
CODE("""m = pd.read_parquet(os.path.join(DRV, 'oa_master.parquet'))
...
MD("""## 5. 관측 OD 로드 + 시간대별 P_obs / A_obs
...
CODE("""r       = adm_g['lp_pool_resident_02_05']
...
CODE("""# scatter — signed proxy A vs P_total + clipped vs P_morning
...
MD("""## 7. signed proxy 공간 분포 — workplace 진단"""),

CODE("""fig, ax = plt.subplots(figsize=(13, 11))
...
MD("""## 8. 회귀 모델 — P_obs / A_obs ~ 인구·구조 변수
...
CODE("""# OLS — log(P_obs + 1) ~ log(features)
...
MD("""## 9. Spatial CV — 옆 admdong 누수 차단
...
CODE("""def cv_score(X, y, splitter, groups=None, label=''):
    rs = []
...
MD("""## 10. 잔차 choropleth — 다음 feature 후보"""),

CODE("""df_model['P_pred_ols'] = ols_P.predict(X)
...
MD("""## 11. pi_aj_v1.parquet 산출 — Lecture 4 입력"""),

CODE("""df_model['P_pred_gbm'] = np.expm1(gbm_P.predict(X_gbm))
...
CODE("""print('OK')
print(f'pi_aj_v1.parquet : {out.shape}')
print(f'결측 (P_obs)     : {out["P_obs"].isna().sum()}')"""),
]

if __name__ == '__main__':
    HERE = os.path.dirname(__file__)
    write_both(cells,
               os.path.join(HERE, '..', 'notebooks', '03_trip_generation.ipynb'),
               os.path.join(HERE, '..', 'slides', '03_trip_generation.md'))
