...
```python
# 시간대별 패턴 보기 — 한 OA 의 24시간 총생활인구 추이
# 종로구의 어떤 OA 하나 골라보기
...
```python
oa_master = gpd.read_parquet(os.path.join(DRV, 'oa_master.parquet'))
...
```python
oa_master['sgg_cd'] = oa_master['TOT_OA_CD'].astype(str).str[:5]
